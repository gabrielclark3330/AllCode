...
# Note to self: The output layer must be the same dimensions as the original image
### MODEL ARCHITECTURE ### Found a basic autoencoder model online
model = Sequential()
...
### MODEL TRAINING ###
es = keras.callbacks.EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=15) # Early stopping (stops training when validation doesn't improve for {patience} epochs)
model_filepath = 'Anomalymodels/testmodel1.keras'  ### REMEMBER TO ADD DIFFERENT FILEPATH OR YOU WILL DELETE LAST MODEL
...
model.fit_generator(
        ...
        validation_freq=1,
        shuffle = True,
        callbacks=[es, save_best])  #tensorboard_callback
...
print("Good prediction percentage for good parts")
print((sum(good_predictions)/len(good_predictions))*100 + "%")
print("Good prediction percentage for bad parts")
print((sum(bad_predictions)/len(bad_predictions))*100 + "%")
